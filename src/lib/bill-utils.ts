...
export function calculateBillAmounts(
  ...
  const otherChargesTotal = Object.values(otherCharges).reduce((sum, charge) => sum + charge, 0);
 ...
 */
export function getSuggestedOtherCharges(productName: string, totalQuantity: number): Record<string, number> {
  const suggestions: Record<string, number> = {};

  // Transport cost calculation (example logic)
  if (totalQuantity > 0) {
    const transportCostPerKg = getTransportCostPerKg(productName);
    suggestions['Transport Cost'] = Math.round(totalQuantity * transportCostPerKg);
  }
...
function getTransportCostPerKg(productName: string): number {
  ...
  if (productName_lower.includes('tomato') || productName_lower.includes('onion')) {
    return 0.5; // ₹0.5 per kg
  } else if (productName_lower.includes('potato') || productName_lower.includes('carrot')) {
    return 0.4; // ₹0.4 per kg
  } else if (productName_lower.includes('leafy') || productName_lower.includes('green')) {
 ...
 */
export function groupQuantitiesByRate(items: Array<{ quantity: number; rate: number }>) {
