...
export default function BillPreviewPage() {
  ...
  const [customCharges, setCustomCharges] = useState<Record<string, Record<string, number>>>({});
  ...
  useEffect(() => {
    ...
    const fetchSessions = async () => {
      try {
        const response = await fetch('/api/sessions?limit=50&sortBy=date&sortOrder=desc');
        if (response.ok) {
          const data = await response.json();
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data && data.data.previews) {
          ...
          setSelectedPreviews(new Set(data.data.previews.map(p => `${p.product_id}_${p.session_id}`)));
          ...
          data.data.previews.forEach(preview => {
            ...
            initialCharges[key] = { ...(preview.suggested_other_charges || {}) };
          });
        ...
        console.error('Failed to fetch bill previews:', response.status, response.statusText);
        setBillPreviews([]);
        setPreviewData(null);
        setSelectedPreviews(new Set());
        setCustomCharges({});
  ...
  const updateCustomCharges = (previewId: string, chargeName: string, amount: number) => {
  ...
  const addCustomCharge = (previewId: string) => {
    ...
    if (chargeName && chargeName.trim()) {
      const amount = parseFloat(prompt('Enter amount (use negative for deductions):') || '0');
      if (!isNaN(amount)) {
  ...
  const generateBills = async () => {
    ...
    try {
      ...
      const requestPreviews = selectedPreviewsArray
        .map(previewId => {
          const preview = billPreviews.find(p => `${p.product_id}_${p.session_id}` === previewId);
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          console.log('Generated bills:', generatedBills.length, 'Errors:', errors.length);
          ...
          if (generatedBills.length > 0) {
            ...
            if (errors.length > 0) {
              let errorMessage = 'Failed to generate bills. The following errors occurred:\n\n';
      ...
      <DesktopOnly>
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Generate Bills</h1>
              <p className="text-slate-600 mt-1">Preview and generate bills for farmers</p>
            </div>
          </div>
              ...
              <div className="space-y-2" ref={farmerDropdownRef}>
                      ...
                      {filteredFarmers.map((farmer) => (
                        <div
                          ...
                          className={`px-3 py-2 hover:bg-gray-100 cursor-pointer ${
                            selectedFarmerId === farmer.id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                          }`}
                          onClick={() => {
                ...
                {selectedFarmerId && (
                  <div className="text-sm text-green-600 mt-1">
                    ✓ Selected: {farmers.find(f => f.id === selectedFarmerId)?.name} - {farmers.find(f => f.id === selectedFarmerId)?.village}
                  </div>
                )}
              </div>
                      ...
                      {filteredSessions.map((session) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setSessionSearchTerm(new Date(session.date).toLocaleDateString());
                            setShowSessionDropdown(false);
                          }}
                        >
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    Preview Summary - {previewData.farmer.name} ({previewData.farmer.village})
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <div className="space-y-4">
                {billPreviews.map((preview) => {
                  ...
                  return (
                    <Card key={previewId} className={isSelected ? 'ring-2 ring-blue-500' : ''}>
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <input
                              type="checkbox"
                              checked={isSelected}
                              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handlePreviewSelection(previewId, e.target.checked)}
                              className="w-4 h-4 text-blue-600 border-gray-300 rounded"
                            />
                            <div>
                              <CardTitle className="text-lg">{preview.product_name}</CardTitle>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg">
                          <h4 className="font-medium mb-3">Items ({preview.items.length})</h4>
                          <div className="space-y-2">
                            {preview.items.map((item) => (
                              <div key={item.id} className="flex justify-between text-sm">
                                <span>{item.quantity} {item.unit} @ ₹{item.rate}/{item.unit}</span>
                          ...
                          <div className="border-t pt-2 mt-2">
                            <div className="flex justify-between font-medium">
                              <span>Total: {preview.total_quantity} {preview.items[0]?.unit} ({preview.total_bags} bags)</span>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-3">
                            ...
                            <div className="space-y-2 max-h-32 overflow-y-auto">
                              {Object.entries(customCharges[previewId] || {}).map(([chargeName, amount]) => (
                                <div key={chargeName} className="flex items-center gap-2 bg-gray-50 p-2 rounded">
                                  <Input
                                    value={chargeName}
                                    disabled
                                  ...
                                  <Input
                                    type="number"
                                    value={amount}
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateCustomCharges(previewId, chargeName, parseFloat(e.target.value) || 0)}
                                  ...
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => removeCustomCharge(previewId, chargeName)}
                                    className="px-2"
                                  >
                              ...
                              {Object.keys(customCharges[previewId] || {}).length === 0 && (
                                <div className="text-xs text-gray-500 text-center py-2">
                                  No additional charges. Click &quot;Add Charge&quot; to add custom charges or deductions.
                                </div>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="bg-blue-50 p-4 rounded-lg">
                            ...
                            <div className="space-y-2 text-sm">
                              ...
                              <div className="flex justify-between">
                                <span>Commission ({preview.commission_rate}%):</span>
                                <span className="text-red-600">-₹{preview.commission_amount}</span>
                              </div>
                              {Object.entries(customCharges[previewId] || {}).map(([name, amount]) => (
                              ...
                              <div className="border-t pt-2 flex justify-between font-semibold">
                                ...
                                <span className="text-green-600">₹{calculateNetPayable(preview, previewId)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        {/* Notes */}
                        <div className="space-y-2">
                          <Label htmlFor={`notes-${previewId}`}>Notes (Optional)</Label>
                          <textarea
                            id={`notes-${previewId}`}
                            placeholder="Add any notes for this bill..."
                            value={notes[previewId] || ''}
                            onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNotes(prev => ({ ...prev, [previewId]: e.target.value }))}
                            rows={2}
                  ...
                  <CardContent>
                    <div className="flex items-center space-x-6">
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <label htmlFor="markAsPaid" className="text-sm font-medium text-gray-700">
          ...
          {!previewData && !loading && selectedFarmerId && (
            <Card>
              <CardContent className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No Unbilled Items Found</h3>
                <p className="text-gray-500">
                  The selected farmer has no auction items that can be billed at this time.
                </p>
              </CardContent>
            </Card>
          )}
