...
export default function BillDetailsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          console.log('Bill refreshed, payment status:', data.data.bill.payment_status);
        } else {
  ...
  const handleMarkAsPaid = async (paymentMethodToUse: string) => {
    ...
    setMarkingPaid(true);
    try {
      console.log('Marking bill as paid:', bill.id, 'Method:', paymentMethodToUse);
    ...
    setPrinting(true);
    try {
      const response = await fetch(`/api/bills/${bill.id}/print?format=${format}`);
      if (response.ok) {
  ...
  const groupItemsByRate = (items: BillDetails['auction_items']) => {
    const groups = new Map<number, { quantities: number[], bags: number, amount: number }>();
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">The bill you&apos;re looking for doesn&apos;t exist or has been deleted.</p>
  ...
  const rateGroups = bill.auction_items ? groupItemsByRate(bill.auction_items) : [];
  const sessionDate = bill.auction_items && bill.auction_items.length > 0 ? bill.auction_items[0].session?.date : bill.created_at;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Bill {bill.bill_number}</h1>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <p className="font-semibold">{bill.farmer?.name || 'Unknown Farmer'}</p>
                <p className="text-sm text-gray-500">{bill.farmer?.village || 'Unknown Village'} • {bill.farmer?.phone || 'No Phone'}</p>
              </div>
              <Separator />
              <div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">{bill._count?.auction_items || (bill.auction_items?.length || 0)} items</p>
                </div>
              </div>
              <Separator />
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">Commission ({bill.commission_rate}%):</span>
                  <span className="font-medium text-red-600">-{formatCurrency(bill.commission_amount)}</span>
                </div>
                {Object.entries(bill.other_charges || {}).map(([charge, amount]) => (
                ...
                <div className="flex justify-between text-lg font-semibold">
                  ...
                  <span className="text-green-600">{formatCurrency(bill.net_payable)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {Array.isArray(rateGroups) && rateGroups.map((group: { rate: number; quantities: number[]; total_quantity: number; bags: number; amount: number }, index: number) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-center mb-3">
                    <div>
                      <h4 className="font-medium">Rate: ₹{group.rate}/kg</h4>
                      <p className="text-sm text-gray-500">{group.bags} bags</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold">{formatCurrency(group.amount)}</p>
                      <p className="text-sm text-gray-500">{group.total_quantity} kg total</p>
                    </div>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    <p className="text-sm text-gray-600">
                      <strong>Quantities:</strong> {group.quantities.join(' kg, ')} kg
                    </p>
                  </div>
                </div>
              ))}
            ...
            <CardContent className="space-y-3">
              ...
              {bill.updated_at !== bill.created_at && (
                <div>
                  ...
                  <p className="font-medium">{formatDateTime(bill.updated_at)}</p>
                </div>
              )}
              {bill.payment_date && (
                <div>
                  <Label className="text-sm font-medium text-gray-500">Payment Date</Label>
                  <p className="font-medium">{formatDateTime(bill.payment_date)}</p>
                </div>
              )}
            </CardContent>
          </Card>
