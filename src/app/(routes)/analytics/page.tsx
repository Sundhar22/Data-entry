...
export default function AnalyticsPage() {
  ...
  const queryResults = useQueries({
    queries: [
      {
        queryKey: ["bill-stats", startDateStr, endDateStr],
        queryFn: async () => {
          const res = await fetch(
            `/api/bills/statistics?start_date=${startDateStr}&end_date=${endDateStr}`,
          );
          if (!res.ok) throw new Error("bill-stats request failed");
      ...
      },
      {
        ...
        queryFn: async () => {
          ...
          if (!res.ok) throw new Error("farmers request failed");
          const json = await res.json();
      ...
      },
      {
        ...
        queryFn: async () => {
          ...
          if (!res.ok) throw new Error("buyers request failed");
          const json = await res.json();
  ...
  const billStats: BillStats | null = billStatsQuery.data ?? null;
  const [showCharts, setShowCharts] = useState(false);

  useEffect(() => {
    if (!isLoading && (billStatsQuery.data || farmersQuery.data || sessionsQuery.data)) {
      const anyWindow = window as unknown as { requestIdleCallback?: (cb: () => void) => void };
      if (typeof anyWindow.requestIdleCallback === "function") {
        anyWindow.requestIdleCallback(() => setShowCharts(true));
      } else {
        setTimeout(() => setShowCharts(true), 0);
      }
    }
  }, [isLoading, billStatsQuery.data, farmersQuery.data, sessionsQuery.data]);
  ...
  useMemo(() => {
    if (
      ...
      const billStatsData = billStatsQuery.data;
      const aggregated: AnalyticsData = {
        ...
        totalBills: billStatsData.overview?.total_bills || 0,
        totalRevenue: billStatsData.overview?.total_amount || 0,
        totalCommission: billStatsData.overview?.commission_amount || 0,
        paidBills: billStatsData.overview?.paid_bills || 0,
        unpaidBills: billStatsData.overview?.unpaid_bills || 0,
      };
  ...
  // Legacy fetch logic removed in favor of TanStack Query
