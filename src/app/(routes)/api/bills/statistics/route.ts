 ...
 */
async function getBillStatisticsHandler(
  ...
  try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const processedPaymentMethodStats = processQueryResults(
    ...
    const avgBillAmount =
      totalBillsCount > 0 ? totalAmount / totalBillsCount : 0;
    const paymentRate =
    ...
    const response = createSuccessResponse({
      overview: {
        ...
        commission_amount: convertBigIntToNumber(
          totalBills._sum.commission_amount,
        ),
