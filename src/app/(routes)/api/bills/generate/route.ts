...
async function generateBillsHandler(req: AuthenticatedRequest): Promise<NextResponse> {
    ...
    await prisma.$transaction(async (tx) => {
        console.log('Starting transaction, processing', previews.length, 'previews');
        for (let i = 0; i < previews.length; i++) {
            const preview = previews[i];
            const billNumber = billNumbers[i];
            console.log('Processing preview:', preview, 'with bill number:', billNumber);
            try {
                ...
                if (existingBill) {
                    console.log('Bill already exists for:', preview.product_id, preview.session_id);
                    errors.push({
                        product_id: preview.product_id,
                        session_id: preview.session_id,
                ...
                console.log('Found auction items:', auctionItems.length, 'for preview:', preview.product_id);

                if (auctionItems.length === 0) {
                    console.log('No auction items found for:', preview.product_id, preview.session_id);
                    errors.push({
                        product_id: preview.product_id,
                        session_id: preview.session_id,
                ...
                console.log('Bill created successfully:', bill.id, 'with number:', bill.bill_number);
                ...
                console.log('Updated', auctionItems.length, 'auction items with bill_id:', bill.id);
                ...
                console.log('Added bill to createdBills array, total now:', createdBills.length);

            } catch (error) {
                console.error('Error creating bill:', error);
                console.log('Error details:', error instanceof Error ? error.message : 'Unknown error', 
                            error instanceof Error && 'code' in error ? (error as {code: string}).code : 'No error code');
    ...
    console.log('Transaction completed. Created bills:', createdBills.length, 'Errors:', errors.length);
    
    // Detailed error logging
    if (errors.length > 0) {
        console.log('Errors during bill generation:', JSON.stringify(errors, null, 2));
    }
