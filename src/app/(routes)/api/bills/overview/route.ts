 ...
 */
async function getBillOverviewHandler(
  ...
  try {
    const [billsStats, unpaidItemsStats, recentActivity, pendingBillsData] =
      await Promise.all([
        // Bills statistics
    ...
    const unpaidItemsEstimatedValue = unpaidItemsWithValue.reduce(
      (total, item) => {
    ...
    const totalBilledAmount = convertBigIntToNumber(
      billsStats._sum.net_payable,
    );
    const paidAmount = convertBigIntToNumber(paidBills._sum.net_payable);
    const unpaidBilledAmount = convertBigIntToNumber(
      unpaidBills._sum.net_payable,
    );
    const totalCommission = convertBigIntToNumber(
      billsStats._sum.commission_amount,
    );
    ...
    return createSuccessResponse({
      overview: {
        ...
        avg_bill_amount:
          totalBillsCount > 0
            ? Math.round(totalBilledAmount / totalBillsCount)
            : 0,

        // Unbilled items
        unbilled_items_count: unbilledItemsCount,
        unbilled_estimated_value: Math.round(unpaidItemsEstimatedValue),
        total_quantity_unbilled: convertBigIntToNumber(
          unpaidItemsStats._sum?.quantity || 0,
        ),
      },
      ...
      alerts: {
        ...
        // Financial alerts
        potential_revenue_loss: Math.round(
          unpaidItemsEstimatedValue + unpaidBilledAmount,
        ),
        aging_unpaid_bills: pendingBillsData.filter((bill) => {
          const daysSinceCreation =
            (Date.now() - new Date(bill.created_at).getTime()) /
            (1000 * 60 * 60 * 24);
