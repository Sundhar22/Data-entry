 ...
 */
async function getBillPreviewHandler(
  ...
  const auctionItems = await prisma.auctionItem.findMany({
    ...
    orderBy: [{ session: { date: "desc" } }, { product: { name: "asc" } }],
  });
  ...
  return createSuccessResponse({
    ...
    summary: {
      ...
      total_gross_amount: billPreviews.reduce(
        (sum, preview) => sum + preview.gross_amount,
        0,
      ),
      total_net_payable: billPreviews.reduce(
        (sum, preview) => sum + preview.net_payable,
        0,
      ),
    },
  });
