...
import {
  ...
  AuctionItemFilterSchema,
} from "@/schemas/auction-item";
...
async function getSessionItemsHandler(
  ...
  const filterParams = {
    ...
    sortBy:
      (searchParams.get("sortBy") as "created_at" | "rate" | "quantity") ||
      "created_at",
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch auction items: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
async function createSessionItemHandler(
  ...
  try {
    ...
    throw new Error(
      `Failed to create auction item: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
