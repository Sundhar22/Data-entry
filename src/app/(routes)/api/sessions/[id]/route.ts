...
async function calculateSessionAnalytics(sessionId: string) {
  ...
  const financialStats = await prisma.$queryRaw<
    Array<{
  ...
  const participantsStats = await prisma.$queryRaw<
    Array<{
  ...
  const overview = overviewStats[0] || {
    ...
    paid_items: BigInt(0),
    unpaid_items: BigInt(0),
  };
  const financial = financialStats[0] || {
    total_revenue: 0,
    total_commission: 0,
    paid_items_count: BigInt(0),
  };
  const participants = participantsStats[0] || {
    total_farmers: BigInt(0),
    total_buyers: BigInt(0),
  };
  ...
  const categoryBreakdownObject = categoryBreakdown.reduce(
    (acc, item) => {
  ...
  return {
    ...
    financial: {
      ...
      average_item_value:
        paidItemsCount > 0 ? totalRevenue / paidItemsCount : 0,
    },
    participants: {
...
async function updateSessionByIdHandler(
  ...
  if (
    ...
    validatedData.status !== existingSession.status &&
    ...
    if (
      ...
      existingSession.date < startOfToday
    ) {
...
async function deleteSessionByIdHandler(
  ...
  if (existingSession.payment_status === "COMPLETED") {
    ...
    throw new ConflictError(
      `Cannot delete PENDING session with ${hasAuctionItems} auction items. Remove items first or complete payments.`,
    );
  } else {
    throw new ConflictError(
      `Cannot delete session with payment status: ${existingSession.payment_status}. Only COMPLETED sessions or empty PENDING sessions can be deleted.`,
    );
  }
