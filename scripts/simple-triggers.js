...
try {
  ...
  function runSQL(sql, description) {
    ...
    try {
      const result = execSync(
        `docker exec -i next_postgres psql -U postgres -d commissioner_db -c "${sql}"`,
        {
          encoding: "utf8",
        },
      );
  ...
  function runSQLFile(file, description) {
    ...
    try {
      execSync(
        `docker exec -i next_postgres psql -U postgres -d commissioner_db < database/triggers/${file}`,
        {
          stdio: "inherit",
        },
      );
